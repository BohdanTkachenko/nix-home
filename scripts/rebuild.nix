...
pkgs.writeShellScriptBin "rebuild" ''
  ...
  if sudo /run/current-system/sw/bin/nixos-rebuild switch --flake "path:$FLAKE_DIR" "''${OVERRIDE_ARGS[@]}" "$@"; then
    exit 0
  fi
  ...
  exec ${pkgs.systemd}/bin/systemd-run --user --pty --collect sudo /run/current-system/sw/bin/nixos-rebuild switch --flake "path:$FLAKE_DIR" "''${OVERRIDE_ARGS[@]}" "$@"
''
