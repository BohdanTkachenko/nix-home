...
When managing and rebuilding this NixOS configuration, agents MUST use the custom rebuild script packages declared in this flake.
...
*(Alternatively, when inside the `devShell` via `direnv` or `nix develop`, the direct aliases `rebuild` or `rebuild-boot` can be used).*

### Why Only This Way Works (Sandbox Escape Mechanics)

The IDE/editor environment spawns AI agents inside an unprivileged process tree where the Linux kernel's **`no-new-privileges`** flag is strictly set. This flag prevents any traditional setuid privilege escalation (like direct `sudo` or `pkexec`), making direct system activation impossible.

To bypass this restriction, our rebuild scripts implement a highly robust **D-Bus transient user service escape**:

1. **Decoupled Execution**: The script calls `${pkgs.systemd}/bin/systemd-run --user --pty --collect sudo /run/current-system/sw/bin/nixos-rebuild switch --flake path:$FLAKE_DIR "$@"`.
2. **Sandbox Escape**: This makes an IPC call to the systemd user manager daemon (`systemd --user`). Because this daemon was spawned by systemd PID 1 at user login, it is outside the IDE's process tree and **does not** inherit the `no-new-privs` flag.
3. **Passwordless Elevation**: The user manager daemon spawns `sudo` under its unconstrained tree. Sudo authenticates immediately without asking for a password thanks to the NOPASSWD wheel rule configured in [security.nix](file:///home/dan/.config/nix/nixos/security.nix).
4. **Real-time Streaming**: The `--pty` flag attaches a pseudo-terminal that streams the stdout/stderr of the build back in real-time, allowing the agent to monitor compilation progress and exit with the correct shell status code.
...
When creating a commit in this repo, always use Conventional Commits.
...
```text
feat(lang): add Polish language Commit message with multi-paragraph body and
multiple footers fix: prevent racing of requests
